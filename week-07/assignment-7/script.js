...
data.then(function(rows){
	...
	d3.select('#cost-vs-sqft')
		.on('click', function(){
			...
			/* YOUR CODE HERE*/
			costVsSqftChart(rows.slice(0,2500));//dot element
		});
	...
	d3.select('#per-sqft-vs-borough')
		.on('click', function(){
			d3.event.preventDefault();
			/* YOUR CODE HERE*/
			costpersqftVsborough(rows.slice(0,1000));
		})
...
function costVsSqftChart(data){
	...
	/*
 	 * Complete this function
	 * YOUR CODE HERE*/
	 //to make up fpr deficit
	 //enter selection
	 const nodesEnter = nodes.enter()// the X unmber of empty spaces to make up defict
	    .transition()
	    .append('circle')
	    .attr('class', 'node')//tdo;
	 ...
	 //exit selection
	  ...
	  const nodesExit = nodes.exit() //seletion
	     .remove();
